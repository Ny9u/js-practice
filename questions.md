...
### 页面加载白屏的衡量指标，怎么让页面加载更快 (字节一面)

> 衡量指标:
>
> 1.首字节时间TTFB(浏览器发起请求到接收到服务器返回的第一个字节所花费的时间)
>
> 2.首次内容绘制时间FCP(浏览器首次绘制任何文本、图像的时间)
>
> 3.可交互时间TTI(页面达到完全可交互状态的时间)
> 加载优化:
>
> 1.从代码本身出发: 可以优化文件体积(通过一些工具去删除文件空格,将多个文件合并成一个文件)
>
> 2.从资源角度出发: 对图片等资源进行压缩或者懒加载,对脚本资源进行异步加载
>
> 3.从网络的角度出发: 使用CDN进行加速, 利用浏览器缓存减少重复请求

### 页面白屏错误排查方案

> 可以从两个方面进行思考: 用户端和服务端
>
> 用户端则从网络,代码,浏览器兼容性这几个方面去思考:
>
> 网络: 资源是否下载下来了
>
> 代码: 通过检查看代码执行是否有报错
>
> 浏览器: 是否用的之前的缓存文件 , 换个浏览器看是否出现问题
>
> 服务端则从nginx,接口,打包产物去思考:
>
> nginx:nginx配置的静态资源是否有问题
>
> 接口:接口是否正常
>
> 打包:打包产物是否完整

### 为什么要用 vnode，为什么不直接修改真实dom (字节二面)

> 1.减少直接操作真实DOM的开销,操作真实DOM会引起浏览器的重排重绘
>
> 2.通过diff算法可以提高更新的效率,将多次更新累积成一次,这样可以避免全量更新
>
> 3.提供跨平台的能力,虚拟DOM是对真实DOM的一种抽象描述,脱离了浏览器的限制
>
> 4.一般情况下虚拟DOM的性能会更好

### computed的更新机制 (字节二面)

> 依赖追踪: computed在首次访问的时候会调用getter函数,执行过程中收集的依赖会收集到computed内部的Watcher中,后续访问时都是访问其缓存值
>
> 懒更新: 当依赖的数据发生变化时,computed不会立即重新计算,Watcher会将其标记为脏数据,下一次getter调用时才会进行更新并更新缓存
